...
// Database connection (JSON file-based)
...
// Helper functions for job operations (replacing MongoDB services)
const saveJobToDB = (job, userId) => {
  try {
    const jobData = {
      ...job,
      userId: userId
    };
    db.insert('jobs', jobData);
...
const updateJobInDB = (jobId, updates) => {
  try {
...
const loadRecentJobsFromDB = () => {
  try {
    // Load recent jobs into memory cache
    const recentJobs = db.findWithOptions('jobs', {}, {
      sort: { createdAt: -1 },
      limit: 100
    });
    ...
    console.log(`✅ Loaded ${recentJobs.length} recent jobs from database`);
    return Promise.resolve();
  } catch (error) {
...
const savePlacesToDB = (jobId, keyword, places, userId) => {
  try {
...
app.post('/api/scrape', requireAuthOrApiKey, scrapeLimiter, async (req, res) => {
    ...
    saveJobToDB(job, req.user._id).catch(err => {
...
app.post('/api/scrape/single', requireAuthOrApiKey, scrapeLimiter, async (req, res) => {
    ...
    saveJobToDB(job, req.user._id).catch(err => {
        logger.error('Failed to save job to database', { jobId, error: err.message });
        // Continue anyway since job is in memory
    });
