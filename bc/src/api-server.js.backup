...
// In-memory data stores (Maps) - MUST BE BEFORE loadExistingResults
const jobs = new Map();
...
function updateJobProgress(jobId, current, total, meta = {}) {
...
async function startScrapingJob(jobId, keywords, jobConfig) {
    const job = jobs.get(jobId);
    ...
    updateJobProgress(jobId, 0, keywords.length);
    ...
    try {
        ...
        const jobResults = await processKeywords(
            ...
            jobConfig?.linkWorkers,
            {
                ...
                onKeywordComplete: ({ keyword, index, total, resultsCount, error, results: keywordResults }) => {
                    ...
                    updateJobProgress(jobId, completedCount, total, { keyword, resultsCount, error });
                }
            }
        );
        ...
        updateJobProgress(jobId, keywords.length, keywords.length);
